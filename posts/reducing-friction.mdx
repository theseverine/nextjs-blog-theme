...
test('test validation', () => {
    const table: [ string, number, boolean ][] = [
        ...
        [ 'zero should fail', 0, false ],
        [ 'non-integers should fail', 0.5, true ],
        [ 'maximum value should succeed', 127, true ],
        [ 'too high value should fail, 128, false ],
    ];

    test.each(table)(_, num, ret) => expect(module.isMidiPitch(num)).toBe(ret);
});
```
