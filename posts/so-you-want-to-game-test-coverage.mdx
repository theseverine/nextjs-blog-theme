...
describe('testing Puppy creation', () => {
    ...
    test('the two puppies should not be identical', () => {
        expect(chloe).not.toStrictEqual(puppy);
    });
});
```
...
And if you write the success/failure tests, then get distracted by a long meeting or a production incident that was caused by code that had 100% test coverage but didn't work, it's actually not implausible that you could have forgotten the need to test return values and moved on, seduced by the 100% code coverage metric.
